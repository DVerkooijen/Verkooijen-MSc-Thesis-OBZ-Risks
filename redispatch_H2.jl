...
function redispatch!(a::Model)
    ...
    b.ext[:constraints][:con1] = @constraint(b, con1[t in T], sum(sum(UP[g,t]*GENCAP[g] for g in g_in_n[n]) - sum(DOWN_H2[h,t]*H2CAP[h] for h in h2_in_n[n]) for n in N) == sum(sum(DOWN[g,t]*GENCAP[g] for g in g_in_n[n]) + sum(UP_H2[h,t]*H2CAP[h] for h in h2_in_n[n]) + curt_delta[n,t] for n in N))
    ...
    b.ext[:constraints][:con2] = @constraint(b, con2[l in L, t in T], F_afterRD[l,t] + TC[l] >= 0 ) #constraint 5c lower bound (Kenis, 2023)
    b.ext[:constraints][:con3] = @constraint(b, con3[l in L, t in T], TC[l] - F_afterRD[l,t] >= 0 ) #constraint 5c upper bound (Kenis, 2023)
    b.ext[:constraints][:con4] = @constraint(b, con4[g in G, t in T], (1-v_DA[g,t]) - UP[g,t] >= 0 ) #constraint 5e (Kenis, 2023)
    b.ext[:constraints][:con5] = @constraint(b, con5[g in G, t in T], v_DA[g,t] - DOWN[g,t] >= 0 )  #constraint 5f (Kenis, 2023)
    b.ext[:constraints][:con6] = @constraint(b, con6[l_dc in L_DC, t in T], TC_DC[l_dc] >= (F_DC_DA[l_dc,t]+F_DC_delta[l_dc,t]) >= -TC_DC[l_dc] ) #constraint 5d (Kenis, 2023)
    b.ext[:constraints][:con7] = @constraint(b, con7[l_dc in L_DC, t in T], - F_DC_DA[l_dc,t] <= F_DC_delta[l_dc,t] <= TC_DC[l_dc] - F_DC_DA[l_dc,t]) #constraint 5h (Kenis, 2023)
    b.ext[:constraints][:con8] = @constraint(b, con8[n in N, t in T], -curt_DA[n,t] <= curt_delta[n,t] <= RES[t,findfirst(N .== n)]-curt_DA[n,t] )  #constraint 5g (Kenis, 2023)
    ...
    if read_data == "5-node_HM" || read_data == "5-node_OBZ"
        ...
        b.ext[:constraints][:con9] = @constraint(b, con9[t in T], -F_DC_DA[1,t]-F_DC_delta[1,t]-(RES[t,findfirst(N .== 5)]-curt_DA[5,t]-curt_delta[5,t]) + (H2CAP[1]*e_DA[1,t] + H2CAP[1]*UP_H2[1,t] - H2CAP[1]*DOWN_H2[1,t]) - F_DC_DA[2,t] - F_DC_delta[2,t] == 0)     #constraint 5i (Kenis, 2023)
    ...
    elseif read_data == "Schonheit_HM" || read_data == "Schonheit_OBZ"  || read_data == "Schonheit_OBZ_adjusted"
        ...
        b.ext[:constraints][:con10] = @constraint(b, con10[t in T], -F_DC_DA[2,t]-F_DC_delta[2,t]-(RES[t,findfirst(N .== 119)]-curt_DA[119,t]-curt_delta[119,t]) + (H2CAP[1]*e_DA[1,t] + H2CAP[1]*UP_H2[1,t] - H2CAP[1]*DOWN_H2[1,t]) +F_DC_DA[8,t] +F_DC_DA[7,t] +F_DC_delta[8,t] +F_DC_delta[7,t] == 0)
        b.ext[:constraints][:con11] = @constraint(b, con11[t in T], -F_DC_DA[5,t]-F_DC_delta[5,t]-(RES[t,findfirst(N .== 120)]-curt_DA[120,t]-curt_delta[120,t]) + (H2CAP[2]*e_DA[2,t] + H2CAP[2]*UP_H2[2,t] - H2CAP[2]*DOWN_H2[2,t]) -F_DC_DA[7,t] +F_DC_DA[9,t] -F_DC_delta[7,t] +F_DC_delta[9,t] == 0)
        b.ext[:constraints][:con12] = @constraint(b, con12[t in T], -F_DC_DA[6,t]-F_DC_delta[6,t]-(RES[t,findfirst(N .== 124)]-curt_DA[124,t]-curt_delta[124,t]) + (H2CAP[3]*e_DA[3,t] + H2CAP[3]*UP_H2[3,t] - H2CAP[3]*DOWN_H2[3,t]) -F_DC_DA[8,t] -F_DC_DA[9,t] -F_DC_delta[8,t] -F_DC_delta[9,t] == 0)
    ...
    elseif read_data == "Reference_Case"
        ...
        b.ext[:constraints][:con12] = @constraint(b, con12[t in T], -F_DC_DA[2,t]-(RES[t,findfirst(N .== 119)]-curt_DA[119,t]-curt_delta[119,t]) 
        # + (H2CAP[1]*e_DA[1,t] + H2CAP[1]*UP_H2[1,t] - H2CAP[1]*DOWN_H2[1,t]) #Put off in case of onshore electrolyser
        +F_DC_DA[8,t] +F_DC_DA[7,t] == 0)

        b.ext[:constraints][:con13] = @constraint(b, con13[t in T], -F_DC_DA[5,t]-(RES[t,findfirst(N .== 120)]-curt_DA[120,t]-curt_delta[120,t]) 
        # + (H2CAP[2]*e_DA[2,t] + H2CAP[2]*UP_H2[2,t] - H2CAP[2]*DOWN_H2[2,t]) #idem
        -F_DC_DA[7,t] +F_DC_DA[9,t] == 0)

        b.ext[:constraints][:con14] = @constraint(b, con14[t in T], -F_DC_DA[6,t]-(RES[t,findfirst(N .== 124)]-curt_DA[124,t]-curt_delta[124,t]) 
